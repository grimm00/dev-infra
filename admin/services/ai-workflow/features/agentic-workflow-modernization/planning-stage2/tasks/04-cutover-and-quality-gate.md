...
### Replication note (model comparison)

**Run:** composer-2-fast (Cursor), 2026-05-02. **Baseline for comparison:** PR #89 (same `develop` base, prior medium-thinking run). File-level deliverables mirror PR #89 for A/B review (**token cost vs quality**).
...
**CI (local — `bats tests/unit/*.bats tests/integration/*.bats`):** 184 tests pass, 4 intentional skips, 0 failures (composer run, 2026-05-02).
...
- [x] CI passes after cutover (184 tests pass, 4 skips, template-sync validates)
