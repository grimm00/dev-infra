...
1. The docs/integration/version-pinned.md examples still reference v0.1.0—please update those download URLs and checksums to v0.1.1 for consistency.
...
3. The validation step excludes admin/, .cursor/, .github/, and start.txt but still leaves hidden files like .gitignore or .dockerignore—consider adding those to your exclusion list to keep distributions truly user-focused.
...
- **Issue:** version-pinned.md examples reference v0.1.0 instead of v0.1.1
- **Solution:** Updated all version references in docs/integration/version-pinned.md to v0.1.1
- **Status:** ✅ Fixed
- **Commit:** f7e2671
...
- **Status:** ✅ Improved with clearer documentation
...
- **Tracking:** Consider for v0.2.0 if file list grows significantly
...
**Impact:** 🟢 LOW - This is an intentional feature, not a bug  
**Effort:** 🟢 LOW
...
**Impact:** 🔴 CRITICAL - Workflow fails completely  
**Effort:** 🟢 LOW
...
**Solution:**
Use `github.event.release.tag_name` instead of `GITHUB_REF` for release events. This contains the actual tag name (e.g., "v0.1.1").
...
| ----------- | ----------- | ----------- | ------- | ----------------------------------------------------------------------------- |
| Sourcery #1 | 🟡 MEDIUM   | 🟡 MEDIUM   | 🟢 LOW  | Documentation consistency - examples should match current release             |
| Sourcery #2 | 🟢 LOW      | 🟡 MEDIUM   | 🟠 HIGH | Good suggestion but current approach is explicit and maintainable             |
| Sourcery #3 | 🟢 LOW      | 🟢 LOW      | 🟢 LOW  | These files are useful for users setting up projects - intentionally included |
| Bugbot #1   | 🟢 LOW      | 🟢 LOW      | 🟢 LOW  | Intentional design - improved with better documentation                       |
| Bugbot #2   | 🔴 CRITICAL | 🔴 CRITICAL | 🟢 LOW  | **BLOCKING** - Version extraction fails, workflow broken                      |
