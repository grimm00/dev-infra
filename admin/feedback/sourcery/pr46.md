...
**Description**: In this block you always check for `$old_version`/`$new_version` with a `v` prefix, but `package.json` only ever gets the bare semver (e.g. `1.4.0`). As a result:
...
**Description**: In this dry-run branch, `grep -n "$old_version" "$file"` won’t find matches in `package.json`, since that file uses `1.4.0` instead of `v1.4.0`. As a result, the preview shows no matches even though an update would occur. To align the preview with the actual behavior, reuse the same `search_version` logic as the `grep -q` check and map to the bare semver for `package.json` before grepping here.
...
**suggestion (bug_risk):** Dry-run match preview does not handle package.json’s bare version format
...
**Description**: Here and in similar tests (e.g., `accepts --dry-run flag`, `accepts --verbose flag`), allowing both `status == 0` and `status == 1` means the test still passes if valid input is incorrectly treated as an error. Once the script’s behavior is finalized, please assert a single expected exit code per case (and split tests if needed for pre/post implementation) so that incorrect exit behavior is caught reliably.
...
**Description**: This and similar tests that branch on the real `.cursor/rules/main.mdc` file and a hardcoded `v1.4.0` will become no-ops once the repo version changes, either skipping or not exercising real behavior. To keep them deterministic and meaningful, consider using fixtures under `$BATS_TEST_TMPDIR` (e.g., copy the file there and adjust its contents) so the tests control the file contents independently of the repo’s current version.
...
EOF
    ...
    # run &quot;$SCRIPT&quot; --old-version v1.4.0 --new-version v1.5.0 --files &quot;$FIXTURE_FILE&quot;
    # assert_success
    # grep -q &quot;\*\*Version:\*\* v1.5.0&quot; &quot;$FIXTURE_FILE&quot;
    # grep -q &quot;\*\*Version:\*\* v1.4.0&quot; &quot;$FIXTURE_FILE.bak&quot;
...
**Description**: This stub currently just sets up a file and leaves a TODO. Given the script’s backup/rollback responsibilities, please add tests that (1) verify a `.bak` file is created with the original content, and (2) simulate a failure during update to confirm rollback leaves files consistent and returns a non-zero exit code. This will ensure the documented recovery behavior is actually enforced by tests.
...
@test &quot;update-version-references.sh creates backup before modification&quot; {
    ...
    [ -f &quot;${TEST_FILE}.bak&quot; ]
    grep -q &quot;\*\*Version:\*\* v1.4.0&quot; &quot;${TEST_FILE}.bak&quot;

    # Original file should have been updated to the new version
    grep -q &quot;\*\*Version:\*\* v1.5.0&quot; &quot;$TEST_FILE&quot;
}
...
@test &quot;update-version-references.sh rolls back changes on failure&quot; {
    ...
    echo &quot;**Version:** v1.4.0&quot; &gt; &quot;$TEST_FILE_OK&quot;
...
**issue (bug_risk):** Relative links in the Related Documentation section appear to have an extra `admin/` segment and likely won’t resolve correctly.
...
- In `.github/workflows/create-release-tag.yml`, consider adding explicit inputs for `branch` and/or `version` for the `workflow_dispatch` path so dry‑run/manual testing can exercise the same validation logic as the PR path rather than hardcoding `main`.
...
|---------|----------|--------|--------|--------|-------|
| #1 | 🟢 **LOW** | 🟢 **LOW** | 🟢 **LOW** | **Already Fixed** | False positive - this was already fixed in PR #45 (commit aff00b9). Script uses `get_version_number()` for package.json checks. |
| #2 | 🟢 **LOW** | 🟢 **LOW** | 🟢 **LOW** | **Already Fixed** | False positive - this was also addressed in PR #45. Dry-run logic handles package.json correctly. |
| #3 | 🟠 **HIGH** | 🟠 **HIGH** | 🟡 **MEDIUM** | **Defer** | Workflow manual dispatch will fail. Need to add version/branch inputs. Not blocking release (feature works via PR path). |
| #4 | 🟡 **MEDIUM** | 🟡 **MEDIUM** | 🟠 **HIGH** | **Defer** | Placeholder tests - some TODOs remain. Tests exist but don't exercise all update logic. High effort for full coverage. |
| #5 | 🟡 **MEDIUM** | 🟡 **MEDIUM** | 🟢 **LOW** | **Defer** | Test exit codes too permissive. Should assert specific expected status. Test quality improvement. |
| #6 | 🟡 **MEDIUM** | 🟡 **MEDIUM** | 🟡 **MEDIUM** | **Defer** | Tests depend on real repo state (brittle). Should use fixtures. Affects test maintainability. |
| #7 | 🟡 **MEDIUM** | 🟡 **MEDIUM** | 🟡 **MEDIUM** | **Defer** | Missing rollback/restore tests. Feature exists but not fully tested. Important but not blocking. |
| #8 | 🟢 **LOW** | 🟢 **LOW** | 🟢 **LOW** | **Fix now** | Broken relative links in RELEASE-NOTES.md. Extra `admin/` segment. Quick fix. |
| #9 | 🟢 **LOW** | 🟢 **LOW** | 🟢 **LOW** | **Fix now** | Broken relative links in VERSION-REFERENCES.md. Missing `../` for admin paths. Quick fix. |
