...
**Description**: In `update_file_version`, the presence check uses `grep -q "$old_version"`, but for `package.json` this is a v-prefixed value (e.g., `v1.4.0`) while the file stores the bare SemVer (e.g., `1.4.0`). That means the grep fails and the function returns early, so `package.json` is never updated. Consider either special-casing `package.json` to search for `get_version_number "$old_version"`, or moving the presence check into the per-file-type branches so each can use the appropriate version format.
...
**Description**: These tests currently only create files and assert their existence; they never call `update-version-references.sh` (the `run "$SCRIPT"...` lines are still TODOs), so they don’t verify the advertised update/validation behavior.
...
|---------|----------|--------|--------|--------|-------|
| #1 | 🔴 **CRITICAL** | 🔴 **CRITICAL** | 🟡 **MEDIUM** | **Fix now** | Bug: package.json never updated due to v-prefix mismatch in grep check |
| #2 | 🟡 **MEDIUM** | 🟡 **MEDIUM** | 🟠 **HIGH** | **Defer** | Tests are placeholders (TODOs), but script works. Need proper integration tests |
| #3 | 🟡 **MEDIUM** | 🟡 **MEDIUM** | 🟢 **LOW** | **Defer** | Tests accept 0 or 1 exit codes - should be more specific |
| #4 | 🟡 **MEDIUM** | 🟡 **MEDIUM** | 🟡 **MEDIUM** | **Defer** | Missing rollback/restore tests - feature exists but not tested |
| #5 | 🟡 **MEDIUM** | 🟡 **MEDIUM** | 🟡 **MEDIUM** | **Defer** | Tests depend on real repo state (v1.4.0) - brittle |
| #6 | 🟢 **LOW** | 🟢 **LOW** | 🟢 **LOW** | **Fix now** | Typo: Missing dot in path (`cursor/rules` → `.cursor/rules`) |
| Overall-#1 | 🔴 **CRITICAL** | 🔴 **CRITICAL** | 🟡 **MEDIUM** | **Fix now** | Same as #1 - package.json v-prefix issue |
| Overall-#2 | 🟡 **MEDIUM** | 🟡 **MEDIUM** | 🟢 **LOW** | **Defer** | Argument parser allows flags as values - minor robustness issue |
